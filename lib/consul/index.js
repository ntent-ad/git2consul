...
var process_records = function(branch, records, cb) {
  ...
  records.forEach(function(record) {
    ...
    // XXX need to make sure source_root is a directory in record.path
    if (branch.source_root) {
      var startsWithSourceRoot = record.path.indexOf(branch.source_root) === 0;
      if( !startsWithSourceRoot ) {
         logger.trace('Skipping record path (does not start with source root): %s', record.path);
// XXX used to be return check_pending(), but this decremented the pending_record count
        return ;
      }

      var isDirectory = record.path.charAt(branch.source_root.length) === "/";
      if( !isDirectory ) {
        logger.trace('Skipping record path (not source root match) %s', record.path);
// XXX used to be return check_pending(), but this decremented the pending_record count
        return ;
      }


// XXX changed to just return
      //return check_pending();
      //return;
    };

    logger.trace('processing record at path: %s', record.path);
    switch (record.type) {
