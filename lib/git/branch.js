...
function Branch(repo_config, name) {
  ...
  Object.defineProperty(this, 'reset_ref_on_cache_error', {value: repo_config['reset_ref_on_cache_error'] === null ? true : repo_config['reset_ref_on_cache_error'] === true});
}
...
Branch.prototype.cloneGitRepo = function(success,failure) {
  var this_obj = this;
...
Branch.prototype.startFromScratch = function(err, success, failure) {
  ...
  this_obj.purgeBranchCache(function(purge_err) {
    ...
    if( this_obj.reset_ref_on_cache_error === true ) {
        // Reset the most recently processed ref: we don't know the state of the repo, so pretend
        // we've never seen this repo before.
        consul_broker.setLastProcessedRef(this_obj, null, function(set_last_processed_error) {
          if (set_last_processed_err) return failure_cb(set_last_processed_err);
          this_obj.cloneGitRepo(success, failure);
        });
    } 
    else
    {
        logger.info("reset REF on cache error is set to FALSE, an attempt to diff will be used if the local cache isn't present");
        this_obj.cloneGitRepo(success, failure);
    }
